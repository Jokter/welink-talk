...
通过指定的 WeLink 用户私聊，在手机上调用电脑中的 Pi coding agent。所有有效消息必须以 `/` 开头，其他消息会被忽略。
...
2. 配置允许控制 Pi 的 WeLink 用户，默认是 `p_xiaoluban`。
...
程序每5秒只查询指定用户最新1条私聊消息，并使用消息 ID 去重。请等待上一条指令被接收后再发送下一条，避免在一个轮询周期内连续发送多条导致遗漏。
