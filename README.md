...
每次轮询会读取最近5条消息，按发送者、接收者和消息 ID 过滤，然后只处理过滤结果中时间最新的1条。其余已扫描消息不会执行，也不会在后续重复处理。

程序以 `w00789509` 的登录身份，每5秒查询它与 `p_xiaoluban` 会话中的最近5条消息，同时校验 `sender=w00789509` 和 `receiver=p_xiaoluban`，最后只取时间最新的1条。Pi 只生成最终答案；回复不经过 Pi 决策或 `welink-cli`，而由桥接程序直接调用 MCP，以 `p_xiaoluban` 身份发送。
